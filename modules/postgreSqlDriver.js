...
export default class PostgreSqlDriver {
	...
	connect(config) {
		return new Promise((resolve, reject) => {
			if (!config.port)
				reject({ err: `config.port must be provded` });
	...
	parse(data) {
		...
		do {
			...
			switch (type) {
				case 'R':
					this.processPassword(offset, length);
					break;
				case 'E':
					this.processError(offset, length);
					break;
				case 'Z':
					this.socket.emit('readyForQuery');
					break;
				case 'T':
					this.processFields(offset);
					break;
				case 'D':
					this.processRow(offset);
					break;
				case 'C':
	...
	processPassword(offset, rLength) {
		let start = offset;
		...
		return {
			...
			bind_params : function(...args) {
				...
				this.statement = this.statement.replace(/(=\?|= +\?)/gm, (s,v) => {
					if(args[0][i -1] == 's')
						return	`='${args[i++].replace(/['"]+/g, '')}'`;
					else if (args[0][i -1] == 'i') {
						if(typeof args[i] === 'string')
							return	`=${Math.floor(Number(args[i++].replace(/['"]+/g, '')))}`;
						else
							return `=${Math.floor(args[i++])}`;
					}
					else if (args[0][i -1] == 'd') {
						if(typeof args[i] === 'string')
							return	`=${Number(args[i++].replace(/['"]+/g, ''))}`;
						else
							return `=${args[i++]}`
					}
			...
			execute : async function() {
				let res = await this.driver.query(this.statement);
				return res;
			}
		}
	}
